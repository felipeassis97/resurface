...
The Material 3 token foundation for Resurface — a calm, autonomy-supportive digital-wellbeing app ("surfacing for air"). Defines fixed light/dark color schemes (incl. a custom `success` role), typography mapped to the two bundled fonts (Fraunces + Plus Jakarta Sans), and shape/elevation/spacing/motion tokens. Product ethics are encoded in the tokens: gentle intervention uses warm amber, never alarm-red. Implemented as Jetpack Compose Material3 theme code under `com.resurface.app.ui.theme`.
