...
The permission model and status checking for Resurface — the set of permissions the app needs, how each is requested (special Settings-intent vs runtime dialog), and live grant-status checking with on-resume re-evaluation. Also covers the stub accessibility service that must be declared so the user can enable accessibility during onboarding. Implemented as `AppPermission` + `PermissionChecker` consumed by the onboarding launch gate.
