...
First-run flow and launch gating for Resurface — the Welcome → Disclosure → Consent → Grant-permissions steps, consent recording (flag + timestamp + disclosure version, on-device), and the start-up routing decision between the onboarding flow and the main app based on persisted consent plus live permission status. Implemented with Jetpack Compose, a Hilt `AppViewModel` launch gate, and DataStore Preferences persistence.
