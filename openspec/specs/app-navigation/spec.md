...
The top-level app shell and navigation for Resurface — the three primary destinations (Home, Insights, Settings), type-safe routing between them, single-source selection state, reselection/back-stack behavior, and the adaptive navigation container that maps destinations to a bottom bar / rail / drawer by window size. Implemented as Jetpack Compose with Navigation Compose (type-safe routes) and Material 3 `NavigationSuiteScaffold`, rendered inside `ResurfaceTheme`.
